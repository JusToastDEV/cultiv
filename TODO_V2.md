...
- [~] **World map UX cleanup** — In progress: replace the one-line tile bar with a richer world HUD/tile inspector, keep the current tile auto-focused on arrival, and stop dead-end “no zones found” failures when a tile has local map intel but no dedicated D1 zone seed yet.
...
- [~] **Tile rendering** — Client renders the map as a CSS grid. Each tile: a color-coded square with a terrain icon (text glyph or small emoji). Player position: highlighted tile. Roads connect tiles. Fog of war: undiscovered tiles shown as `?` until visited. Current pass: richer tile inspector, nearby tile callouts, clearer arrival state, and stronger map readability.
...
- [~] **Resource nodes on tiles** — Spirit herb nodes, ore veins, and beast lairs tied to specific tiles. Have respawn timers. When a player arrives at a tile with a resource node, show it as an interactive element. Harvesting depletes it for that player for the respawn window. Current pass: surface tile-level resource intel in the world inspector while the full server harvest/explore backend catches up.
