...
- [~] **Remove "City Affairs" button** — In progress: city hooks are being folded into the World surface and settlement services are being exposed from city tiles instead of a dedicated City tab.
...
- [~] **World map UX cleanup** — In progress: replace the one-line tile bar with a richer world HUD/tile inspector, keep the current tile auto-focused on arrival, stop dead-end “no zones found” failures when a tile has local map intel but no dedicated D1 zone seed yet, and make travel visibly in-transit instead of an instant static snap.
...
- [~] **Tile rendering** — Client renders the map as a CSS grid. Each tile: a color-coded square with a terrain icon (text glyph or small emoji). Player position: highlighted tile. Roads connect tiles. Fog of war: undiscovered tiles shown as `?` until visited. Current pass: richer tile inspector, local tile sub-sites, transit marker visuals, organized quest/presence cards, and stronger map readability.
